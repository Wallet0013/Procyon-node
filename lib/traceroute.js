...
internals.Traceroute.trace = function (host,hop,timeout, callback) {
    ...
    Dns.lookup(host.toUpperCase(), (err) => {
        ...
        const command = (internals.isWin ? 'tracert -d ' : 'traceroute -q 1 -n -w ' + timeout + ' -m' + hop  + ' ') + host;
...
internals.parseOutput = function (output) {
    ...
    for (let i = 0; i < lines.length; ++i) {
        const value_hop = (Object.keys(internals.parseHop(lines[i]))[0]) ?
        Object.keys(internals.parseHop(lines[i]))[0] : null;
        const value_ms  = (Object.values(internals.parseHop(lines[i]))[0]) ?
        Object.values(internals.parseHop(lines[i]))[0][0] : null;
        hops.push( {
            number  : i+1,
            hop     : value_hop,
            msec      : value_ms
